...
function emojifyPhrase(str, emoji) {
    ...
    const replaceChars = layer =>
        layer.replace(/[s]/g, spacer).replace(/[x]/g, `:${emoji}:`);
    let emojified = [...str].map(char =>
        letterMap[char.toUpperCase()].map(layer => replaceChars(layer), [])
    );
...
console.log(emojifyPhrase("yes!", "success"));
